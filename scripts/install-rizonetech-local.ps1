...
try {
  ...
  & wsl.exe --cd $wslChromeDest -- bash -lc "chmod +x bin/chromemcp-run .codex/scripts/chromemcp-run.py 2>/dev/null || true" | Out-Null
  $token = (& wsl.exe -- bash -lc "MCP_TOKEN_PATH=~/.config/chromemcp-codex/token chromemcp token 2>/dev/null || true" 2>$null) -join ""
  $token = $token.Trim()
} catch {
  $token = ""
}
...
if ([string]::IsNullOrWhiteSpace($token) -or $token.Length -lt 32) {
  Write-Warning "Could not generate Codex ChromeMCP auth token during install. Leaving placeholder .mcp.json; run 'MCP_TOKEN_PATH=~/.config/chromemcp-codex/token chromemcp token' (after installing ChromeMCP) and update .mcp.json before enabling MCP auth."
} else {
  $mcpConfig = @{
    mcpServers = @{
      "chromemcp-playwright" = @{
        ...
        note = "Codex-isolated ChromeMCP Playwright MCP server. Start it with 'chromemcp codex-up'. Token generated by 'MCP_TOKEN_PATH=~/.config/chromemcp-codex/token chromemcp token'. Install ChromeMCP from https://github.com/rizonetech/ChromeMCP."
      }
    }
  }
