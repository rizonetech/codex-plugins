...
try {
  ...
  $token = (& wsl.exe -- bash -lc "chromemcp token 2>/dev/null || true" 2>$null) -join ""
  $token = $token.Trim()
} catch {
  $token = ""
}
...
if ([string]::IsNullOrWhiteSpace($token) -or $token.Length -lt 32) {
  Write-Warning "Could not generate ChromeMCP auth token during install. Leaving placeholder .mcp.json; run 'chromemcp token' (after installing ChromeMCP) and update .mcp.json before enabling MCP auth."
} else {
  $mcpConfig = @{
    mcpServers = @{
      "chromemcp-playwright" = @{
        ...
        note = "Local ChromeMCP Playwright MCP server. Start it with 'chromemcp up'. Token generated by 'chromemcp token'. Install ChromeMCP from https://github.com/rizonetech/ChromeMCP."
      }
    }
  }
